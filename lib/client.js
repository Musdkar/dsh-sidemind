window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BtwOverlay({ sessionId, useBtw, follow, close }) {
      ...
      const complete = Boolean(answer) && !view.streaming && view.messages.some(message => message.role === 'assistant')
      const titleQuestion = question.replace(/\s+/g, ' ').trim()
      const title = titleQuestion ? `/btw ${titleQuestion}` : '/btw'
      ...
      const onPanelKeyDown = event => {
        ...
        const page = Math.max(80, Math.floor((bodyRef.current?.clientHeight ?? 160) * .82))
      ...
      return h('aside', {
        ref: panelRef,
        className: `sidemind-btw${complete && overflowing ? ' sidemind-btw-scrollable' : ''}`,
        role: 'dialog',
        tabIndex: -1,
        'aria-label': title,
        onKeyDown: onPanelKeyDown,
      },
        h('div', { className: 'sidemind-btw-title', title }, title),
        h('div', { className: 'sidemind-btw-hints' },
          overflowing ? h('span', { className: 'sidemind-btw-scrollhint', 'aria-hidden': true }, '↑↓') : null,
          h('button', {
            type: 'button',
            className: 'sidemind-btw-esc',
            onClick: dismiss,
            'aria-label': 'Dismiss side question',
            title: 'Dismiss',
          }, '[Esc]')
        ),
        h('div', { className: 'sidemind-btw-body', ref: bodyRef, onScroll: updateOverflow },
          view.error
            ? h('div', { className: 'sidemind-btw-error' }, view.error)
            : answer
              ? h('div', { className: 'sidemind-btw-answer' },
                  h(MarkdownBody, { text: answer, streaming: Boolean(view.streaming) }))
              : h('div', { className: 'sidemind-btw-pending' },
                  h('span', { className: 'sidemind-btw-spinner', 'aria-hidden': true }, '⠋'),
                  h('span', null, 'Answering…')
                )
        )
      )
    }
    ...
    function installStyles() {
      ...
      style.textContent = `
        ...
        .sidemind-btw { --sidemind-btw-accent: var(--dsw-alias-warning, #c89a3d); position: absolute; left: 50%; bottom: calc(100% + 10px); transform: translateX(-50%); width: min(720px, calc(100% - 24px)); z-index: 40; color: var(--dsw-alias-label-primary, inherit); background: var(--dsw-alias-background-primary, #fff); border: 1px solid var(--dsw-alias-separator, rgba(127,127,127,.28)); border-radius: 10px; box-shadow: 0 10px 32px rgba(0,0,0,.16), 0 1px 4px rgba(0,0,0,.07); outline: 0; }
        .sidemind-btw-scrollable:focus { border-color: var(--sidemind-btw-accent); box-shadow: 0 10px 32px rgba(0,0,0,.16), 0 0 0 1px color-mix(in srgb, var(--sidemind-btw-accent) 28%, transparent); }
        .sidemind-btw-title { position: absolute; top: -9px; left: 10px; max-width: calc(100% - 112px); padding: 0 6px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; background: var(--dsw-alias-background-primary, #fff); color: var(--sidemind-btw-accent); font-size: 12px; line-height: 18px; font-weight: 650; }
        .sidemind-btw-hints { position: absolute; top: -9px; right: 8px; display: flex; align-items: center; gap: 6px; padding: 0 4px; background: var(--dsw-alias-background-primary, #fff); color: var(--dsw-alias-label-tertiary, #999); font-size: 10px; line-height: 18px; user-select: none; }
        .sidemind-btw-scrollhint { letter-spacing: .05em; }
        .sidemind-btw-esc { border: 0; padding: 0; color: inherit; background: transparent; font: inherit; line-height: inherit; cursor: pointer; }
        .sidemind-btw-esc:hover, .sidemind-btw-esc:focus-visible { color: var(--dsw-alias-label-primary, inherit); font-weight: 650; outline: none; }
        .sidemind-btw-body { max-height: min(19.5em, 46vh); overflow-y: auto; overscroll-behavior: contain; padding: 14px 14px 12px; scrollbar-gutter: stable; }
        .sidemind-btw-answer { min-height: 20px; line-height: 1.56; }
        .sidemind-btw-pending { min-height: 24px; display: flex; align-items: center; gap: 8px; color: var(--dsw-alias-label-tertiary, #999); }
        .sidemind-btw-spinner { display: inline-block; color: var(--sidemind-btw-accent); transform-origin: center; animation: sidemind-btw-spin .8s steps(8, end) infinite; }
        .sidemind-btw-error { color: var(--dsw-alias-error, #d35b5b); line-height: 1.5; white-space: pre-wrap; overflow-wrap: anywhere; }
        ...
        @keyframes sidemind-btw-spin { to { transform: rotate(360deg); } }
        @keyframes sidemind-pulse { 0%,100% { opacity: .25; transform: scale(.82); } 50% { opacity: .72; transform: scale(1); } }
        @media (max-width: 720px) { .sidemind-btw { width: calc(100% - 12px); bottom: calc(100% + 6px); border-radius: 9px; } .sidemind-keyhint { display: none; } }
        @media (prefers-reduced-motion: reduce) { .sidemind-spinner, .sidemind-thinking-dot, .sidemind-btw-spinner { animation: none; } .sidemind-transcript { scroll-behavior: auto; } }
      `
