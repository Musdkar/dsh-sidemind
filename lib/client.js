  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      ctx.on('command/executed', (sessionId, name, result) => {
        ...
        if (name === 'side' && meta.kind === 'side') {
          ...
          if (typeof ctx.sidebarRight.openTabIn === 'function') {
            ctx.sidebarRight.openTabIn(sessionId, TAB_KIND, options)
          } else {
            ctx.sidebarRight.openTab(TAB_KIND, options)
          }
    ...
    function SidePanel({ useTabInfo, sessionId, follow, prompt, close }) {
      ...
      useEffect(() => {
        ...
        // Intentionally do not remove the listener when this body unmounts:
        // inactive sidebar tab bodies may unmount while the tab record remains alive.
        // The occurrence signal itself owns the listener lifetime and fires only when
        // the record disappears (explicit close/replacement) or the plugin unloads.
        return undefined
      ...
      for await (const frame of ctx.remote.session.follow({
        ...
        if (frame.type === 'snapshot') {
          ...
          for (const record of frame.records ?? []) {
            if (record?.type === 'event') events.set(record.event.seq, record.event)
          }
    ...
    async function closeChild(ctx, sessionId, meta) {
      try {
        await sideControl(ctx, sessionId, { op: 'close', id: meta.childId })
      } catch (error) {
    ...
    function useBtwStore(store, selector) {
